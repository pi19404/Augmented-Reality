    ...
    Mat image;
      ...
      //              cerr << center.y <<":" << l.p1.y <<":" << l.p2.y << endl;
                    ...
                    if(tmp.dir==3 || tmp.dir==2 ||tmp.dir==1)
                    {
                          ...
                          /*for(int k=0;k<4;k++)
                          {
                          int index=(tmp.dir+k)%4;
                          cv::circle(image,Point(tmp.vertex[index][0],tmp.vertex[index][1]),10,Scalar(k*30,k*30,0),-1,8);
                          }*/
...
*/
    glTranslatef(px,py,25);//world co-ordinate to pixel co-ordinate
